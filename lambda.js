...
async function requestToken(req, res) {
  ...
  try {
    ...
    res.writeHead(401)
    res.end('Could not get new tokes with refresh token (probably expired). Go to /login')
    return
  }
  await saveRefreshToken(tokens.refresh)
...
const getLoginURL = 
  twitch.getLoginURL.bind(null, 
    process.env.TWITCH_CLIENT_ID, 
    process.env.TWITCH_CLIENT_REDIRECT_URI)
const getTokensWithCode = 
  twitch.getTokensWithCode.bind(null, 
    fetch, 
    process.env.TWITCH_CLIENT_ID, 
    process.env.TWITCH_CLIENT_SECRET, 
    process.env.TWITCH_CLIENT_REDIRECT_URI)
const getTokensWithRefreshToken = 
  twitch.getTokensWithRefreshToken.bind(null, 
    fetch, 
    process.env.TWITCH_CLIENT_ID, 
    process.env.TWITCH_CLIENT_SECRET)
...
const saveRefreshToken = tokenStore.saveRefreshToken.bind(null, 
  MongoClient, process.env.MONGO_URI, process.env.MONGO_DB)
const loadRefreshToken = tokenStore.loadRefreshToken.bind(null, 
  MongoClient, process.env.MONGO_URI, process.env.MONGO_DB)